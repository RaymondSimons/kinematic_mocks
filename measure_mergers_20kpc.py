...
#gals = ['VELA09', 'VELA10']
#gals = ['VELA14', 'VELA15', 'VELA16', 'VELA17']
#gals = ['VELA19', 'VELA20', 'VELA21', 'VELA22']
gals = ['VELA23', 'VELA24', 'VELA25', 'VELA26']
gals = ['VELA27', 'VELA28', 'VELA29', 'VELA30']
gals = ['VELA32', 'VELA33', 'VELA34', 'VELA35']
gals = ['VELA30']
gals = ['VELA27']
gals = ['VELA28']
...
for g, gal in enumerate(gals):
    rec_cat = np.loadtxt('../catalogs/recenter/recenter_%s.cat'%gal, skiprows = 12)
    if make_cat: 
        m_cat = open('../catalogs/mergers/%s.cat'%gal, 'w+')
    ...
    for s, scale in enumerate(scales):
        ...
        if len(rec_c) > 0:
            ...
            young_mn = nan
            seed(1)
            mom_fl = glob('/Volumes/wd/vela_v2/momentum_measurements/%s/*%s*momentum.fits'%(gal, scale))
            rec_fl = glob('/Volumes/wd/vela_v2/data/recenter/%s_%s.fits'%(gal, scale))
            ...
            if len(mom_fl) > 0:
                ...
                fits_name = '../stars_mergers/'+gal+'/'+gal+'_a0.'+str(scale)+'_starsmergers.fits'
                ...
                if do_plot:
                    ...
                    fig.tight_layout()
                    savefig('../figures/merger_maps/20kpc/%s_%s.png'%(gal, scale), dpi = 300)
                    plt.close('all')
                if make_cat: 
                    #write young
...
#Parallel(n_jobs = 1, backend = 'threading')(delayed(run_measure_merger)(gal, scales) for gal in gals)
